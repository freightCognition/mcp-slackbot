...
process.env.TOKEN_ENDPOINT_URL = process.env.TOKEN_ENDPOINT_URL || 'http://localhost/token';
...
process.env.SLACK_SIGNING_SECRET = process.env.SLACK_SIGNING_SECRET || 'test-signing';
...
function formatInfractions(infractions) {
  ...
  return infractions.map(infraction => {
    const ruleText = normalizeNullableText(infraction.RuleText, 'Unknown infraction');
...
function buildSlackBlocks(data) {
  const blocks = [
    {
      type: "header",
      text: { type: "plain_text", text: "MyCarrierPortal Risk Assessment", emoji: true }
    },
    {
      type: "section",
      text: {
    ...
    {
      type: "context",
      elements: [{ type: "mrkdwn", text: `Total Points: ${data.RiskAssessmentDetails?.TotalPoints || 'N/A'}` }]
    },
    { type: "divider" }
  ];
  ...
  categories.forEach(category => {
    ...
    if (categoryData) {
      blocks.push(
        {
          type: "section",
          text: { type: "mrkdwn", text: `*${category}:* ${getRiskLevelEmoji(categoryData.TotalPoints)} ${getRiskLevel(categoryData.TotalPoints)}` }
        },
        {
          type: "context",
          elements: [{
...
describe('Risk Assessment Functions', () => {
  ...
  describe('formatInfractions', () => {
    ...
    it('formats single infraction', () => {
      ...
      expect(formatInfractions(infractions)).toBe('- Test Rule: Test Output (100 points)');
    });
    ...
    it('omits output when RuleOutput is null', () => {
      ...
      expect(formatInfractions(infractions)).toBe('- Missing Output Rule (250 points)');
    });
...
describe('Slack Block Building', () => {
  describe('buildSlackBlocks', () => {
    ...
    it('builds blocks for high risk carrier', () => {
      ...
      const categoryBlocks = blocks.filter(b =>
        b.type === 'section' && b.text?.text?.includes('Authority:')
      );
...
describe('formatSlackLinks', () => {
  it('converts HTML anchor tags to Slack links', () => {
    expect(formatSlackLinks('<a href="https://example.com">Click</a>')).toBe('<https://example.com|Click>');
  });
...
describe('Active Assessment Management', () => {
  ...
  it('expires after 5 minutes', () => {
    activeAssessments.set('C_OLD', { userId: 'U1', mcNumber: '123', startedAt: Date.now() - 6 * 60 * 1000 });
