...
slackApp.command("/mcp", async ({ command, ack, respond }) => {
  ...
  while (attempt < maxAttempts) {
    try {
      logger.info(
        { mcNumber, attempt: attempt + 1 },
        "Fetching data for MC number",
      );
      ...
      const mcpData = {
        TotalPoints:
          (data.IsBlocked ? 1000 : 0) +
          (data.FreightValidateStatus === "Review Recommended" ? 1000 : 0),
        OverallRating: getRiskLevel(
          (data.IsBlocked ? 1000 : 0) +
            (data.FreightValidateStatus === "Review Recommended" ? 1000 : 0),
        ),
        Infractions: [],
      ...
      if (
        ...
        if (refreshed.success) {
          ...
          await ephemeral(
            "Error: Could not refresh authentication. Please check logs or contact admin.",
          );
          return;
        }
      } else {
        ...
        if (error.response && error.response.status === 401) {
          userMessage =
            "Authentication failed even after attempting to refresh. Please contact an administrator.";
        }
